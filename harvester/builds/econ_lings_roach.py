...
class EconLingRoach(MlBuild):
    ...
    def state(self) -> List[Union[int, float]]:
        my_lost_m, my_lost_g = self.knowledge.lost_units_manager.calculate_own_lost_resources()
        enemy_lost_m, enemy_lost_g = self.knowledge.lost_units_manager.calculate_enemy_lost_resources()
        enemy_power: ExtendedPower = self.knowledge.enemy_army_predicter.predicted_enemy_power
        return [self.ai.time, self.ai.supply_workers, self.ai.supply_army, self.ai.minerals,
                ...
                self.knowledge.enemy_army_predicter.enemy_known_worker_count,
                self.knowledge.enemy_army_predicter.enemy_mined_minerals,  # 15
                self.knowledge.enemy_army_predicter.enemy_army_known_minerals,
                enemy_power.power,
                ...
                enemy_lost_m,
                enemy_lost_g,
                self.knowledge.rush_distance  # 25
        ]
    ...
    async def chat_space(self):
        ...
        if self.ai.time > 30:
            await self.chatter.chat_taunt_once("actions",
               lambda: f'Actions: 0: bases, 1: drones, 2: queens, 3: lings, 4:roaches')
    ...
    def create_plan(self) -> List[Union[ActBase, List[ActBase]]]:
        ...
        roaches = Step(lambda k: self.action == 4, SequentialList([
            ...
            StepBuildGas(1),
            Step(None, StepBuildGas(2), skip_until=RequiredSupply(20, supply_type=SupplyType.Workers)),
            Step(None, StepBuildGas(3), skip=RequiredGas(300), skip_until=RequiredSupply(30, supply_type=SupplyType.Workers)),
        ...
        hydras = Step(lambda k: self.action == 5, SequentialList([
            ...
            Step(None, StepBuildGas(3), skip_until=RequiredSupply(30, supply_type=SupplyType.Workers)),
            Step(None, StepBuildGas(4), skip=RequiredGas(300),
        ...
        tactics = SequentialList([
            ...
            # PlanGatherOptimizer(),
            OverlordScout(),
            Step(None, WorkerScout(), skip_until=RequiredTime(70)),
            SpreadCreep(),
            InjectLarva(),
            PlanHeatOverseer(),
